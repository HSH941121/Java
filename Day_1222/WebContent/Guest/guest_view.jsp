...
//1.드라이버 로딩
	...
	String sql2 = "update tbl_guest2 set readcnt = readcnt +1 where idx = ?";
	pstmt = conn.prepareStatement(sql2);
	pstmt.setInt(1, idx);
	pstmt.executeUpdate();
	
	String sql = "select * from tbl_guest2 where idx = ?";
	...
	if(rs.getString("modifydate") != null){
		modify = rs.getString("modifydate");	
	}
         ...
         <tr>
           <td class="content">
             <p align="right"><font size="2" face="돋움">
              <%=rs.getString("name") %>  / <font size="2" face="돋움"><%=rs.getString("regdate").substring(0,10) %> / 수정한 날짜 : <%=modify%> / <%=rs.getString("readcnt")%>번 읽음</font>
             <p>
